...
ChoreoModeling.prototype.setInitiatingBand = function(element) {
  const initParticipantBand = element.parent.children.find(
    x => x.businessObject.id === element.parent.businessObject.initiatingParticipantRef.id);
  this._commandStack.execute('band.makeInitiating', {
    choreography: element.parent,
    nonInitParticipant: element,
    initParticipant: initParticipantBand,
    nonInitBandKind: element.diBand.participantBandKind,
