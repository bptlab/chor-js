...
ChoreoPasteHandler.prototype.postExecute = function(context) {
  ...
  Object.keys(newElements).forEach(key => {
    ...
    if (elem.copyMessageFlowRef) {
      elem.businessObject.messageFlowRef.forEach(flow => {
        flow.messageRef = createdByOldID[flow.messageRef.id].element;
        flow.messageRef.businessObject.$parent.rootElements.push(flow.messageRef.businessObject);
      });
    }
  });
