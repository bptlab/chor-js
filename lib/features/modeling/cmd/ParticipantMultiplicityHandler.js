...
import { resizeBands } from '../../../util/BandUtil';
...
ParticipantMultiplicityHandler.prototype.updateAllBands = function(participant) {
  ...
  activities.forEach(activity => resizeBands(activity, null, activity.businessObject.di.bounds));
  activities.forEach(activity => this._eventBus.fire('element.changed', { element: activity }));
  let bandShapes = this._elementRegistry.filter(
    element => element.businessObject.id === participant.businessObject.id);
...
ParticipantMultiplicityHandler.prototype.execute = function(context) {
  if (context.hasMultiplicity) {
    ...
    context.participant.businessObject.participantMultiplicity = this._bpmnFactory.create(
      'bpmn:ParticipantMultiplicity', { maximum: 2 });
    context.participant.businessObject.participantMultiplicity.$parent = context.participant.businessObject;
  }
