...
function chooseParticipants(hints, allParticipants) {
  ...
  if (hints && hints.sourceShape) {
    ...
    if (isChoreoActivity(source) || precedingActivities.length === 1) {
      ...
      const count = {};
      const participants = flatten(precedingActivities.map(
        p => p.bandShapes.map(bs => bs.businessObject)));

      participants.forEach(
        bo => {
          const newCount = (count[bo.id] || 0) +1;
          count[bo.id] = newCount;
          count.first = newCount > (count[count.first] || 0) ? bo.id : count.first;
          count.second = bo.id !== count.first && newCount > (count[count.second] || 0) ? bo.id: count.second;
        });

      const mostLikelyInitiating = participants.find(bo => bo.id === count.first);
      const mostLikelyReceiving = participants.find(bo => bo.id === count.second);
      recommended.initiator = mostLikelyInitiating;
      recommended.receiver = mostLikelyReceiving;
    }
  }
  return recommended;
