...
import {
  getBandHeight, updateBands, updateBandsBounds
} from '../../../util/BandUtil';
  ...
  this.preExecuted('shape.create', function(event) {
    ...
    if (is(shape, 'bpmn:ChoreographyActivity') && shape.type !== 'label') {
      ...
      if (shape.oldBusinessObject) {
        ...
        // Info: CreateShapeHandler.js will set these positions again, but we set them here already to determine the
        // bounds for the bands
        shape.x = position.x;
        shape.y = position.y;
        ...
        //updateBandsBounds(shape);


        // This is required for correct positioning of the shape by CreateShapeHandler.js
        ...
        let proxy = new Proxy(shape, handler);
        shape = proxy;
        context.shape = shape;
        ///////////////
      */
    ...
    if (is(shape, 'bpmn:ChoreographyActivity') && shape.type !== 'label') {
      ...
      if (shape.oldBusinessObject) {
        shape.bandShapes.forEach(bandShape => {
          ...
          commandStack.execute('shape.create', { shape: bandShape, position: context.position, parent: shape });
        });
      }
    }
  });
