  ...
  this.executed('shape.create', function(event) {
    ...
    if (is(shape, 'bpmn:ChoreographyActivity') && shape.type !== 'label') {
      ...
      if (!shape.oldBusinessObject) {
        ...
        if (shape.copyMessageFlowRef) {
          shape.copyMessageFlowRef.forEach(flow => {
            const newFlow = createMessageFlowSemantics(injector, shape.businessObject, flow.sourceRef);
            newFlow.messageRef.name = flow.messageRef.name;
            linkMessageFlowSemantics(injector, shape.businessObject, newFlow);
          });
        }
      }
    }
