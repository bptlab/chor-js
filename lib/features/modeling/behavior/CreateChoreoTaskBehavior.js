...
import { getConnectedElements, isInitiating } from '../../../util/DiagramWalkerUtil';

function chooseParticipants(hints, allParticipants) {
  const recommended = {};
  if (hints && hints.sourceShape) {
    const source = hints.sourceShape;
    if (is(source, 'bpmn:ChoreographyActivity')) {
      // We switch the participants of the previous activity.
      const participants = source.bandShapes;
      recommended.initiator = (isInitiating(participants[0]) ? participants[1] : participants[0]).businessObject;
      recommended.receiver = (isInitiating(participants[0]) ? participants[0] : participants[1]).businessObject;
    } else {
      const predecessors = getConnectedElements(hints.sourceShape, direction, hasRequiredType);
      // Todo
    }
  } else {
    // In the default case no source shape was given, e.g. because the the activity was dragged from the side pallet.
    recommended.initiator = allParticipants[0];
    recommended.receiver = allParticipants[1];
  }
  return recommended;
