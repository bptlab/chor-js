...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  if (is(element, 'bpmn:Participant')) {
    ...
    if (this._rules.allowed('band.canBeMadeInitiating', { participant: element })) {
      assign(actions, {
        'band.makeInitiating': {
          group: 'edit',
          ...
          action: {
            click: () => self._modeling.setInitiatingBand(element)
          }
        }
      });
    }
  ...
  // --------------------------------------------------------------------------------------------
  // replace sequence flows with default sequence flows and vice versa
  if (is(businessObject, 'bpmn:SequenceFlow') && !self._popupMenu.isEmpty(element, 'bpmn-replace')) {
    assign(actions, {
      'replace': {
        group: 'edit',
        className: 'bpmn-icon-screw-wrench',
        ...
        action: {
          click: function(event, element) {
            ...
            self._popupMenu.open(element, 'bpmn-replace', position);
          }
        }
      }
    });
