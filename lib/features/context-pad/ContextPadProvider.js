...
ContextPadProvider.prototype.getContextPadEntries = function(element) {
  ...
  // --------------------------------------------------------------------------------------------
  // Provide a popup for the wrench menu of choreo activities.
  // Can be used to change loop-type or set references for CallChoreos.
  if (is(businessObject, 'bpmn:ChoreographyActivity')) {
    assign(actions, {
      'activityWrench': {
        group: 'edit',
        className: 'bpmn-icon-screw-wrench',
        title: this._translate('Link CallChoreography'),
        action: {
          click: (event, element) => {
            const position = assign(popupMenuPosition(element), {
              cursor: { x: event.x, y: event.y }
            });
            self._popupMenu.open(element, 'activity-wrench-provider', position);
          }
        }
      }
    });
    ...
    if (this._rules.allowed('band.canInitiatingBeSwapped', { bandShape: element })) {
      assign(actions, {
        'band.swapInitiatingParticipant': {
          ...
          title: this._translate('Make Initiating'),
          action: {
  ...
  if (is(businessObject, 'bpmn:SubChoreography')) {
    assign(actions, {
      'shape.toggleCollapse': {
        ...
        title: this._translate('Collapse'),
        action: {
  ...
  // --------------------------------------------------------------------------------------------
  // change participant linking of call choreographies
  if (is(businessObject, 'bpmn:Participant')
    && is(element.parent, 'bpmn:CallChoreography')
    && element.parent.businessObject.calledChoreographyRef) {
    assign(actions, {
