...
ChoreoRules.prototype.init = function() {
  ...
  this.addRule('element.paste', function(context) {
    ...
    if (is(element, 'bpmn:Message') && element.doNotPaste) {
      // we do not paste messages that are default messages and therefore do not have a shape.
      return false;
    }
    ...
    if (is(parent, 'bpmn:ChoreographyActivity') || (is(element, 'bpmn:Message') && !element.doNotPaste)) {
      return true;
    }
  ...
  this.addRule('band.canBeMadeInitiating', function(context) {
    if (is(context.participant, 'bpmn:Participant')) {
      // TODO: check if participant appears in previous choreo.
      const participant = context.participant;
      return participant.parent.businessObject.initiatingParticipantRef !== participant.businessObject;
    }
    return false;
  });
};
