 ...
 * @constructor
 */
export default function NewChoreoModdleCopy(injector, moddle, eventBus, bpmnFactory) {
  this._moddle = moddle;
  injector.invoke(ModdleCopy, this);
  const LOWER_PRIO = 650; // Must be lower than LOW_PRIORITY in BpmnCopyPaste.js

  // In bpmn-js these event listners are attached in BpmnCopyPaste which shares the module with ModdleCopy,
  // however, they are not really decoupled anyways. To avoid having to overwrite that class as well, we simply attache
  // it here.
  eventBus.on('copyPaste.copyElement', LOWER_PRIO, (context) => {
    ...
    // We need to trick a bit with the messageFlowRef. We store the name of the messages—new messages will be created
    // later by the CreateChoreoTaskBehavior—because its the only important info about messages which are not copied themselves.
    // We also store direction and visibility. Unfortunately, context hints does not work or we would have to change much more code.
    ...
    console.log(descriptor.oldBusinessObject);
  });

}
inherits(NewChoreoModdleCopy, ModdleCopy);

NewChoreoModdleCopy.$inject = [
  'injector',
  'moddle',
  'eventBus',
  'bpmnFactory'
...
NewChoreoModdleCopy.prototype.copyProperty = function(property, parent, propertyName) {
...
NewChoreoModdleCopy.prototype.copyElement = function(sourceElement, targetElement, propertyNames) {
