...
function bpmnDescriptor(element, descriptor) {
  ...
  if (element.hidden) {
    // hide elements that are currently hidden (e.g., elements in a collapsed Sub Choreo)
  ...
  if (element.bandShapes) {
    // we copy only bands participant ids to tolerate bands being deleted in original object before paste.
...
function pasteElement(canvas, bpmnFactory, bpmnRules, helper, context, elementFactory, moddle, injector) {
  ...
  if (is(descriptor.oldBusinessObject, 'bpmn:ChoreographyActivity')) {
    ...
    if (descriptor.copyMessageFlowRef) {
      descriptor.copyMessageFlowRef.forEach(flow => {
        // Todo: How to remove this on revert?
  ...
  if (descriptor.parent && descriptor.parent.oldBusinessObject &&
    ...
    if (is(newElement.oldBusinessObject, 'bpmn:ChoreographyActivity')
      ...
      // we need to add the di to the root element of the diagram
      let choreo = canvas.getRootElement().businessObject;
      let bpmnDiagram = choreo.di;
      // Todo: how do we remove this on revert?
      bpmnDiagram.get('planeElement').push(descriptor.diBand);
      descriptor.diBand.$parent = bpmnDiagram;
    }
  }
