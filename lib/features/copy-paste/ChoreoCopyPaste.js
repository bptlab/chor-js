 ...
 * @param context
 */
function pasteElement(canvas, bpmnFactory, bpmnRules, helper, context, elementFactory, moddle) {
  ...
  if (is(descriptor.oldBusinessObject, 'bpmn:ChoreographyActivity')) {
    ...
    if (descriptor.copyMessageFlowRef) {
      descriptor.businessObject.messageFlowRef = descriptor.copyMessageFlowRef.map(flow => {
        const newFlow = bpmnFactory.create(flow.$type);
        assign(newFlow, omit(flow, ['$type']));
        delete newFlow.id;
        bpmnFactory._ensureId(newFlow);
        newFlow.$parent.messageFlows.push(newFlow);
        return newFlow;
      });
    }
...
export default function ChoreoCopyPaste(bpmnFactory, eventBus, copyPaste, clipboard, canvas, bpmnRules, elementFactory, moddle) {
  ...
  eventBus.on('element.paste', function(context) {
    ...
    pasteElement(canvas, bpmnFactory, bpmnRules, helper, context, elementFactory, moddle);
  });
