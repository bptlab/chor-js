 ...
 * @param moddle
 */
function copyBandShape(descriptor, bpmnFactory, elementFactory, moddle) {
  const fakeBounds = {};
  const bandDI = bpmnFactory.createDiShape('bpmndi:BPMNShape', fakeBounds, {
    choreographyActivityShape: descriptor.parent.businessObject.di,
...
function elementPasteHandler(canvas, bpmnFactory, bpmnRules, helper, context, elementFactory, moddle) {
  ...
  if (descriptor.parent && descriptor.parent.oldBusinessObject &&
    ...
    if (is(newElement.oldBusinessObject, 'bpmn:ChoreographyActivity')
      ...
      const newBandShape = copyBandShape(descriptor, bpmnFactory, elementFactory, moddle);
      assign(descriptor, newBandShape);
      newElement.bandShapes.push(newBandShape);
