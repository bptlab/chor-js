...
export default class ChoreoSwitch {
  ...
  constructor(eventBus, commandStack, choreoUtil) {
    ...
    // Register to events
    this._eventBus.once('import.render.complete', this.registerHandlers);
  ...
  switchElement() {
    return `
      <div class="djs-select-wrapper">
        ...
        <button id="start-rename-choreography" class="djs-button">R</button>
      ...
      <div class="djs-rename-wrapper">
        ...
        <button id="end-rename-choreography" class="djs-button">Ok</button>
      </div>
    `;
  }

  handleSwitchEvent(e) {
    this._choreoUtil.switchChoreography(e.target.value);
  }
