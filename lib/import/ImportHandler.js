 ...
 * @param useCache {boolean} use cache for elements. This should be set to true to switch between diagrams within a file.
 * @return {Promise} a promise resolving after the rendering
 */
export function displayChoreography(viewer, bpmnDiagramOrId, useCache) {
  return new Promise((resolve, reject) => {
    ...
    if (!useCache) {
      // If we do not use the cache, we have to flush it, to avoid id clashes later on
      shapeCache = {};
    }
