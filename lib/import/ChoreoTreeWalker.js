...
export default function ChoreoTreeWalker(handler, injector) {
  ...
  function handleDefinitions(definitions, choreoID) {
    ...
    if (diagram.plane.bpmnElement.participants) {
      diagram.plane.bpmnElement.participants.forEach(participant => {
        participant.di = injector.get('bpmnFactory').createDiShape(
          'bpmndi:BPMNShape',{}, { bpmnElement: participant });
      });
    }
