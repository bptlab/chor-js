...
export default function ChoreoTreeWalker(handler, translate) {
  ...
  function handleChoreographyActivity(activity, context) {
    // handle participants
    forEach(activity.participantRefs, contextual(handleParticipant, context));
  }

  function handleParticipant(participant, context) {
    let childCtx = visit(participant, context);

    // handle a potential attached message
    if (childCtx.diBand.isMessageVisible) {
      if (is(context, 'bpmn:ChoreographyTask')) {
        let task = context.businessObject;
        if (task.messageFlowRef) {
          task.messageFlowRef.forEach(messageFlow => {
