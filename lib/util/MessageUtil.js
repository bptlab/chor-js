 ...
 * @param injector
 * @param message
 * @param source_participant
 * @param target_participant
 * @returns {bpmn:MessageFlow}
 */
export function createMessageFlow(injector, message, source_participant, target_participant) {
  let moddle = injector.get('moddle');
  let messageFlow = moddle.create('bpmn:MessageFlow', {
    messageRef: message,
    sourceRef: source_participant,
    targetRef: target_participant
  });
  messageFlow.id = moddle.ids.nextPrefixed('MessageFlow_', messageFlow);
