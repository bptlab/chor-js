...
/**
 * Updates all attributes concerning size in shape and DI of the bands according to their
 * activities size.
 * Does not use the event bus, so changes are not immediately displayed.
 *
 * @param {Object} activityShape shape object of the choreography activity
 */
export function updateBandsBounds(activityShape) {

  const newBounds = activityShape.businessObject.di.bounds;
  const bandShapes = activityShape.bandShapes;

  const gapIndex = getBandGapIndex(bandShapes.length);
  const topShapes = bandShapes.slice(0, gapIndex);
  const bottomShapes = bandShapes.slice(gapIndex);
  ...
  topShapes.forEach(bandShape => {
    ...
    assign(bandShape, updatedBounds);
    assign(bandShape.diBand.bounds, updatedBounds);
